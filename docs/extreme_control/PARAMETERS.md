...
| EST | `FTC_EST_DELAY` | 0.04 s | 命令到角响应固定延迟 |
| EST | `FTC_EST_AGE` | 10 s | 效能估计最大年龄 |
| EST | `FTC_THR_MAX` | 0 N | 独立总推力尺度，0 表示质量不可观测 |
...
| REC | `FTC_REC_ACT` | 0 | 唯一 rate 输入仲裁，默认关闭 |
...
参数生成检查当前识别 42 个 `FTC_` 参数。`FTC_MON_EN`、`FTC_CA_EN`、`FTC_REC_EN`、`FTC_REC_ACT`、`FTC_SIM_EN` 均默认 0。
